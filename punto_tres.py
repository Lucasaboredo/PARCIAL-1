...
print("")
print("Punto A")
def ordenado_nombre(jedis):
    def sort_key(jedi):
        return (jedi["name"], jedi["species"])
    return sorted(jedis, key=sort_key)

jedis_ordenados = ordenado_nombre(jedis)
...
for jedi in jedis:
    if jedi["name"] in ["Ahsoka Tano", "Kit Fisto"]:
        print("Mostrar informacion de", jedi["name"] + ":")
        for prop, value in jedi.items():
            print(f"{prop}: {value}")
        print()
...
print("Punto C")
for jedi in jedis:
    if jedi["master"] in ["Yoda", "Luke Skywalker"]:
        print(jedi["name"])
